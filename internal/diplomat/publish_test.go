...
func TestPublish_Success(t *testing.T) {
	...
	tokenKeyStr := base64.StdEncoding.EncodeToString(keyBytes[:])
	...
	plainToken := "alice-mock-auth-value-without-token"
...
func TestPublish_IdempotencyFencing(t *testing.T) {
	...
	rawKey, _ := crypto.ParseKey(tokenKeyStr)
	encToken, err := crypto.Encrypt(rawKey, []byte("mock-auth-value"))
	if err != nil {
		t.Fatalf("failed to encrypt test token: %v", err)
	}
